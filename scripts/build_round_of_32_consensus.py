#!/usr/bin/env python3
"""Build Round of 32 consensus data and the legacy consensus page."""
...
APP_DIR = ROOT / "apps" / "round_of_32_consensus"

CONSENSUS_PATH = GENERATED_DIR / "round_of_32_consensus.json"
...
def build_consensus(knockout_data: dict) -> dict:
    ...
    round_of_32 = next(
        (stage for stage in knockout_data.get("stages", []) if stage.get("stage") == "round_of_32"),
        {},
    )
    match_ids = [str(match_id) for match_id in round_of_32.get("matchIds", [])]
...
def match_consensus(match_id: str, players: list[dict], player_count: int) -> dict:
    match_picks = [
        ...
        if match.get("stage") == "round_of_32" and str(match.get("matchId")) == match_id
    ]
...
def bonus_question_consensus(players: list[dict]) -> list[dict]:
    ...
    for player in players:
        for answer in player.get("bonusAnswers", {}).get("round_of_32", []) or player.get("roundOf32BonusAnswers", []):
...
<head>
  ...
  <title>World Cup Pool Round of 32 Consensus</title>
...
<body>
  ...
  <main class="page">
    <section class="summary-grid" id="metrics" aria-label="Round of 32 consensus metrics"></section>
    ...
    <section class="panel">
      <div class="panel-head">
        <div>
          ...
          <h2>Round of 32 Bonus Consensus</h2>
        </div>
      </div>
  ...
  <script id="round-of-32-consensus-data" type="application/json">{data_script}</script>
